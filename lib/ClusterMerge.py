...
class ClusterMerge(object):
    ...
    def update_tables(self, cur, levels=['t25', 't10', 't5', 't2', 't0']):
        '''
        ...
        sql = "SELECT s.pk_id, s.sample_name, c.t0, c.t2, c.t5, c.t10, c.t25 FROM samples s, sample_clusters c WHERE s.pk_id=c.fk_sample_id AND c."+self.t_level+" IN %s"
        ...
        for r in rows:
            ...
            sql = "INSERT INTO sample_history (fk_sample_id, t25_old, t10_old, t5_old, t2_old, t0_old, t25_new, t10_new, t5_new, t2_new, t0_new, renamed_at) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
            cur.execute(sql ,(r['pk_id'], \
                              r['t25'], r['t10'], r['t5'], r['t2'], r['t0'], \
